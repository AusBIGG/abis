...
|`badvalues` | The message's values are not considered valid according to the value-based validation tests within the BDR. The test(s) that have raised errors will be indicated with the prefix `bdrm:doesNotConformTo`
...
NOTE: At the time of writing, value tests for BDR messages haven't been defined so dummy test IRIs of `req:ValuesReqK` & `req:ValuesReqM` are used here. When defined, real IRIs will be referenced.
